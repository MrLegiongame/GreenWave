...
class Graph:
    ...
    Attributes:
        ...
        graph_for_path (networkx.DiGraph): NetworkX graph for path finding
    """
    ...
    def __init__(self, nodes, edges, vehicles, dt=0):
        ...
        self.graph_for_path = None  # used for path finding logic
        if True:
            self.nodes = nodes
            ...
            for direction in junction.directions:
                for in_lane in direction.in_lanes:
                    ...
                    for to_lane in in_lane.to_lanes:
                        ...
                        graph.add_edge(in_lane_name, out_lane_name, weight=0)
        ...
        for road in self.edges:
            ...
            for out_lane in road.first_direction.out_lanes:
                for in_lane in road.second_direction.in_lanes:
                    ...
                    in_lane_name = "InLane" + str(in_lane.index_in_map)
                    graph.add_edge(out_lane_name, in_lane_name, weight=length)
    ...
    def get_path(self, start, end):
        ...
        try:
            lane_path = nx.shortest_path(self.graph_for_path, source=source, target=target, weight="weight")
        except Exception:
